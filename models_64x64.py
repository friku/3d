...
def generator3d(z, dim=64, reuse=True, training=True):
    bn = partial(batch_norm, is_training=training)
    fc_bn_relu = partial(fc, normalizer_fn=bn, activation_fn=relu, biases_initializer=None)
    ...
    with tf.variable_scope('generator', reuse=reuse):
        ...
        print(y)
        y = tf.layers.conv3d_transpose(y,dim*2,5,2,padding='same',use_bias=True,)
#        y = tf.contrib.layers.bias_add(y)
        y = relu(y)
        y = bn(y)
        y = tf.layers.conv3d_transpose(y,dim*1,5,2,padding='same',use_bias=True)
#        y = tf.contrib.layers.bias_add(y)
        y = relu(y)
        y = bn(y)
        y = tf.layers.conv3d_transpose(y,1,5,2,padding='same',use_bias=True)
#        y = tf.contrib.layers.bias_add(y)
        img = tf.tanh(y)
        return img
    
def discriminator_wgan_gp_3d(img, dim=64, reuse=True, training=True):
    conv_ln_lrelu = partial(conv, normalizer_fn=ln, activation_fn=lrelu, biases_initializer=None)

    with tf.variable_scope('discriminator', reuse=reuse):
        y = tf.layers.conv3d(img,dim*1,5,2,padding='same')
        y = ln(y)
        y = lrelu(y)
        y = tf.layers.conv3d(y,dim*2,5,2,padding='same')
        y = ln(y)
        y = lrelu(y)
        y = tf.layers.conv3d(y,dim*4,5,2,padding='same')
        y = ln(y)
        y = lrelu(y)
        logit = fc(y, 1)
        return logit